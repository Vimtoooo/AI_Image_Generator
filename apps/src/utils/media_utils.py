...
class MediaUtils:
    """Utility class for handling aspect-ration, output paths and math calculations."""
    WIDTH: Final[int] = 720
    ...
    @classmethod
    def calculate_portrait_dimensions(cls, width: int | None = None, height: int | None = None) -> tuple[int, int]:
        """
    ...
    @classmethod
    def define_filename_path(cls, path_to_folder: Path, filename: str, file_type: str) -> Path:
        """
        ...
        if normalized_type not in cls.PERMITTED_FILE_TYPES:
            raise InvalidFileTypeError(
                f"Unsupported file type extension: '{file_type}'. "
                f"Permitted extensions are: {cls.PERMITTED_FILE_TYPES}"
            )
